...
Aluno: Geandria de Menezes Pereira
Data:  27/05/2026
=============================================================
...
OPERACOES_REGISTRY = {
    ...
    "divisao": lambda a, b: a / b if b != 0 else (_ for _ in ()).throw(ZeroDivisionError("Divisão por zero.")),
...
def validar_url(valor) -> tuple[bool, str]:
    s = str(valor)
    valido = s.startswith("http://") or s.startswith("https://")
    valido = valido and "." in s[7:]  # após protocolo
    return (valido, "" if valido else "URL inválida: deve começar com http:// ou https:// e conter domínio")
...
print("\n[DEPOIS]")
casos_depois = casos + [("url", "https://iteam.com.br")]
...
# Handlers (cada um retorna (percentual, motivo))
def handler_cupom_blackfriday(c: Carrinho):
    return (0.30, "Cupom Black Friday: 30%")

def handler_cupom_bemvindo_valido(c: Carrinho):
    return (0.10, "Cupom de boas-vindas: 10%")

def handler_cupom_bemvindo_invalido(c: Carrinho):
    return (0.0, "Cupom BEMVINDO10 apenas para novos clientes")

def handler_vip_alto_valor(c: Carrinho):
    return (0.20, "VIP + compra >= R$500: 20%")

def handler_vip_padrao(c: Carrinho):
    return (0.10, "VIP: 10% base")

def handler_fiel_muitos_itens(c: Carrinho):
    return (0.15, "Fiel + 5 itens: 15%")

def handler_fiel_padrao(c: Carrinho):
    return (0.08, "Fiel: 8%")

def handler_fds_alto_valor(c: Carrinho):
    return (0.12, "FDS + >= R$200: 12%")

def handler_fds_padrao(c: Carrinho):
    return (0.05, "Fim de semana: 5%")

def handler_compra_acima_mil(c: Carrinho):
    return (0.10, "Compra acima de R$1.000: 10%")

def handler_padrao(c: Carrinho):
    return (0.0, "Sem desconto aplicável")

def handler_aniversario_loja(c: Carrinho):
    return (0.25, "Aniversário da loja: 25%")

# Registrar todos
desconto_registry.register("cupom:blackfriday", handler_cupom_blackfriday)
desconto_registry.register("cupom:bemvindo:valido", handler_cupom_bemvindo_valido)
desconto_registry.register("cupom:bemvindo:invalido", handler_cupom_bemvindo_invalido)
desconto_registry.register("cliente:vip:alto_valor", handler_vip_alto_valor)
desconto_registry.register("cliente:vip:padrao", handler_vip_padrao)
desconto_registry.register("cliente:fiel:muitos_itens", handler_fiel_muitos_itens)
desconto_registry.register("cliente:fiel:padrao", handler_fiel_padrao)
desconto_registry.register("fds:alto_valor", handler_fds_alto_valor)
desconto_registry.register("fds:padrao", handler_fds_padrao)
desconto_registry.register("compra:acima_mil", handler_compra_acima_mil)
desconto_registry.register("padrao", handler_padrao)
desconto_registry.register("aniversario_loja", handler_aniversario_loja)

def decidir_regra(c: Carrinho) -> str:
    # cupons
    if c.cupom == "BLACKFRIDAY":
        return "cupom:blackfriday"
    if c.cupom == "BEMVINDO10":
        return "cupom:bemvindo:valido" if c.tipo_cliente == "novo" else "cupom:bemvindo:invalido"
    # cliente vip
    if c.tipo_cliente == "vip":
        return "cliente:vip:alto_valor" if c.valor_total >= 500 else "cliente:vip:padrao"
    # cliente fiel
    if c.tipo_cliente == "fiel":
        return "cliente:fiel:muitos_itens" if c.quantidade_itens >= 5 else "cliente:fiel:padrao"
    # fim de semana
    if c.dia_semana in (5, 6):
        return "fds:alto_valor" if c.valor_total >= 200 else "fds:padrao"
    # compra alta
    if c.valor_total >= 1000:
        return "compra:acima_mil"
    # nova regra (aniversário da loja) - quarta-feira e >= 300
    if c.dia_semana == 3 and c.valor_total >= 300:
...
# Testar a nova regra
carrinho_aniversario = Carrinho(350.0, None, "normal", 3, 2)
pct, motivo = calcular_desconto_depois(carrinho_aniversario)
print(f"\n  [NOVA REGRA] R$ {carrinho_aniversario.valor_total:7.2f} | {pct*100:4.0f}% | {motivo}")
...
class RegistryFull:
    ...
    def get(self, chave):
        if chave not in self._store:
            raise KeyError(f"Chave '{chave}' não encontrada no registry {self.nome}")
        return self._store[chave]
    def update(self, chave, valor):
        self._store[chave] = valor
    def unregister(self, chave):
...
evento_registry = RegistryFull("EventoRegistry")
...
def handle_usuario_deletado(evento: Evento) -> dict:
    return {"evento": evento.tipo, "status": "ok",
            "acoes": ["dados_pessoais anonimizados", "sessoes_ativas encerradas", "log_auditoria registrado"]}
...
def handle_estoque_baixo(evento: Evento) -> dict:
    ...
    qtd = evento.payload.get("quantidade", 0)
    return {"evento": evento.tipo, "status": "ok",
            "acoes": ["alerta_compras enviado", f"reposicao_solicitada: {produto} ({qtd} un)"]}
...
# --- RUNTIME UPDATE (feature flag) ---
@registrar_evento("pagamento.aprovado_v2")  # versão nova, não registra automaticamente no lugar
def handle_pagamento_aprovado_v2(evento: Evento) -> dict:
    acoes = ["pedido_liberado", "nota_fiscal emitida", "webhook_externo: notifica ERP"]
    if evento.prioridade >= 3:
        acoes.append("notificacao_prioritaria enviada")
    return {"evento": evento.tipo, "status": "ok", "acoes": acoes}

# Atualizar em runtime o handler de "pagamento.aprovado"
evento_registry.update("pagamento.aprovado", handle_pagamento_aprovado_v2)

print("\n[FEATURE FLAG] handler de pagamento.aprovado atualizado em runtime!")
print("Testando evento pagamento.aprovado com novo handler:")
evento_pag = Evento("pagamento.aprovado", {"valor": 1500.0}, 3, "webhook")
res = processar_evento_depois(evento_pag)
print(f"  [{res['status'].upper():8}] {evento_pag.tipo:25} → {res['acoes']}")

# Relatório final
print("\n[REGISTRY FINAL] Chaves registradas:")
print(evento_registry.listar_chaves())
