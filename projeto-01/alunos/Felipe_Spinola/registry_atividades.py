...
# Desafio extra: raiz (√a)
OPERACOES_REGISTRY["raiz"] = lambda a, b: math.sqrt(a)
...
def _relatorio_json_pretty(dados: dict) -> None:
    import json
    print(f"[JSON_PRETTY] {json.dumps(dados, indent=2)}")


relatorio_registry = Registry("RelatorioRegistry")
...
def gerar_relatorio_depois(formato: str, dados: dict) -> None:
    try:
        ...
        raise ValueError(f"Formato desconhecido: {formato}") from e
...
validador_registry = Registry("ValidadorRegistry")
...
def validar_url(valor) -> tuple[bool, str]:
    valor_str = str(valor)
    valido = (valor_str.startswith("http://") or valor_str.startswith("https://")) and "." in valor_str
...
print("\n[DEPOIS]")
casos_depois = casos + [("url", "https://iteam.org"), ("url", "invalido")]
...
desconto_registry = Registry("DescontoRegistry")

def decidir_regra(carrinho: Carrinho) -> str:
    # Desafio extra: aniversario_loja (quarta-feira = 3, valor >= 300)
    ...
    if carrinho.cupom == "BLACKFRIDAY":
        ...
        if carrinho.tipo_cliente == "novo":
            return "cupom:bemvindo:valido"
        else:
            return "cupom:bemvindo:invalido"

    elif carrinho.tipo_cliente == "vip":
        if carrinho.valor_total >= 500:
            return "cliente:vip:alto_valor"
        return "cliente:vip:padrao"

    elif carrinho.tipo_cliente == "fiel":
        if carrinho.quantidade_itens >= 5:
            return "cliente:fiel:muitos_itens"
        return "cliente:fiel:padrao"

    elif carrinho.dia_semana in (5, 6):   # fim de semana
        if carrinho.valor_total >= 200:
            return "fds:alto_valor"
        return "fds:padrao"
...
def _desc_blackfriday(carrinho: Carrinho) -> tuple[float, str]:
    return (0.30, "Cupom Black Friday: 30%")

def _desc_bemvindo_valido(carrinho: Carrinho) -> tuple[float, str]:
    return (0.10, "Cupom de boas-vindas: 10%")

def _desc_bemvindo_invalido(carrinho: Carrinho) -> tuple[float, str]:
    return (0.0, "Cupom BEMVINDO10 apenas para novos clientes")

def _desc_vip_alto(carrinho: Carrinho) -> tuple[float, str]:
    return (0.20, "VIP + compra >= R$500: 20%")

def _desc_vip_padrao(carrinho: Carrinho) -> tuple[float, str]:
    return (0.10, "VIP: 10% base")

def _desc_fiel_muitos(carrinho: Carrinho) -> tuple[float, str]:
    return (0.15, "Fiel + 5 itens: 15%")

def _desc_fiel_padrao(carrinho: Carrinho) -> tuple[float, str]:
    return (0.08, "Fiel: 8%")

def _desc_fds_alto(carrinho: Carrinho) -> tuple[float, str]:
    return (0.12, "FDS + >= R$200: 12%")

def _desc_fds_padrao(carrinho: Carrinho) -> tuple[float, str]:
    return (0.05, "Fim de semana: 5%")

def _desc_acima_mil(carrinho: Carrinho) -> tuple[float, str]:
    return (0.10, "Compra acima de R$1.000: 10%")

def _desc_padrao(carrinho: Carrinho) -> tuple[float, str]:
    return (0.0, "Sem desconto aplicável")

def _desc_aniversario_loja(carrinho: Carrinho) -> tuple[float, str]:
    return (0.25, "Aniversário da Loja: 25% (Quarta-feira)")


desconto_registry.register("cupom:blackfriday", _desc_blackfriday)
desconto_registry.register("cupom:bemvindo:valido", _desc_bemvindo_valido)
desconto_registry.register("cupom:bemvindo:invalido", _desc_bemvindo_invalido)
desconto_registry.register("cliente:vip:alto_valor", _desc_vip_alto)
desconto_registry.register("cliente:vip:padrao", _desc_vip_padrao)
desconto_registry.register("cliente:fiel:muitos_itens", _desc_fiel_muitos)
desconto_registry.register("cliente:fiel:padrao", _desc_fiel_padrao)
desconto_registry.register("fds:alto_valor", _desc_fds_alto)
desconto_registry.register("fds:padrao", _desc_fds_padrao)
desconto_registry.register("compra:acima_mil", _desc_acima_mil)
...
print("\n[DEPOIS]")
carrinhos_teste_depois = carrinhos_teste + [
    Carrinho(350.0, None, "normal", 3, 2),  # quarta-feira, >= 300 -> aniversario_loja!
]
for c in carrinhos_teste_depois:
...
class EventoRegistry:
    def __init__(self, nome: str = "EventoRegistry") -> None:
        self._store = {}
        self._nome = nome

    def register(self, chave: str, valor) -> None:
        if chave in self._store:
            raise ValueError(f"[{self._nome}] Chave '{chave}' já cadastrada.")
        self._store[chave] = valor
...
evento_registry = EventoRegistry("EventoRegistry")
...
def handle_sistema_erro(evento: Evento) -> dict:
    resultado = {"evento": evento.tipo, "status": "erro", "acoes": []}
    resultado["acoes"].append("log_erro registrado")
    if evento.prioridade == 4:
        resultado["acoes"].append("oncall_acionado via PagerDuty")
        resultado["acoes"].append("rollback_iniciado")
...
# V2 do handler de pagamento.aprovado para demonstração do runtime update
def handle_pagamento_aprovado_v2(evento: Evento) -> dict:
    resultado = {"evento": evento.tipo, "status": "ok", "acoes": []}
    resultado["acoes"].append("pedido_liberado")
    resultado["acoes"].append("nota_fiscal emitida")
    if evento.prioridade >= 3:
        resultado["acoes"].append("notificacao_prioritaria enviada")
    # Ação exclusiva da V2
    resultado["acoes"].append("webhook_parceiro_notificado")
    return resultado

print("\n--- Demonstração de Runtime Update (Feature Flag) ---")
# Swap em runtime
evento_registry.update("pagamento.aprovado", handle_pagamento_aprovado_v2)
print("  ✅ [FEATURE FLAG] handler de pagamento.aprovado atualizado em runtime!")

# Execução do teste novamente para ver a nova ação no print
evento_teste_aprovado = Evento("pagamento.aprovado", {"valor": 1500.0}, 3, "webhook")
res_v2 = processar_evento_depois(evento_teste_aprovado)
print(f"  [APÓS UPDATE] {evento_teste_aprovado.tipo:25} → {res_v2['acoes']}")

# Imprime relatório final
print("\nEventos registrados no EventoRegistry:")
