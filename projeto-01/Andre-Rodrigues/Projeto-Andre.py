...
OPERACOES_REGISTRY = {
    ...
    "raiz": lambda a, b=None: math.sqrt(a)  
}

def calcular_depois(operacao: str, a: float, b: float = None) -> float:
    if operacao not in OPERACOES_REGISTRY:
        raise ValueError(f"Operação '{operacao}' é inválida ou inexistente.")
    
    try:

        if operacao == "raiz":
            return OPERACOES_REGISTRY[operacao](a)
        return OPERACOES_REGISTRY[operacao](a, b)
    except ZeroDivisionError:
        raise ValueError("Erro: Divisão por zero não é permitida.")

print("Atividade 1 - Nova Calculadora:")
print(calcular_depois("soma", 10, 10))       
print(calcular_depois("divisao", 20, 2))    
print(calcular_depois("raiz", 64))          
...
class Registry:
    ...
    def get(self, name: str):
        if name not in self._registry:
            raise ValueError(f"Formato/Chave '{name}' não encontrado no registro.")
...
def formatar_resumo(dados: dict) -> str:
    return f"Resumo: {len(dados)} itens processados."

def formatar_detalhado(dados: dict) -> str:
    return f"Detalhado: {str(dados)}"

def formatar_csv(dados: dict) -> str:
    return ",".join(dados.keys())

def formatar_html(dados: dict) -> str:
    return f"<div>{str(dados)}</div>"

def formatar_pdf(dados: dict) -> str:
    return f"[PDF Data] {str(dados)}"

# Registros Originais
relatorio_registry.register("resumo", formatar_resumo)
relatorio_registry.register("detalhado", formatar_detalhado)
relatorio_registry.register("csv", formatar_csv)
relatorio_registry.register("html", formatar_html)
relatorio_registry.register("pdf", formatar_pdf)

# Desafio extra - json_pretty
import json
def formatar_json_pretty(dados: dict) -> str:
    return json.dumps(dados, indent=4)

relatorio_registry.register("json_pretty", formatar_json_pretty)

# Função Principal
def gerar_relatorio_depois(formato: str, dados: dict) -> str:
    formatador = relatorio_registry.get(formato)
    return formatador(dados)

# Saída
print("Atividade 2:")
dados_teste = {"id": 1, "nome": "Produto A", "preco": 29.90}
print(gerar_relatorio_depois("resumo", dados_teste))
print(gerar_relatorio_depois("json_pretty", dados_teste))
...
def validar_cpf(valor) -> tuple[bool, str]:
    valido = len(str(valor).replace(".", "").replace("-", "")) == 11
    return (valido, "" if valido else "CPF deve conter 11 dígitos")

@registrar_validador("telefone")
def validar_telefone(valor) -> tuple[bool, str]:
    valido = len(str(valor)) >= 10
    return (valido, "" if valido else "Telefone inválido")

@registrar_validador("cep")
def validar_cep(valor) -> tuple[bool, str]:
    valido = len(str(valor).replace("-", "")) == 8
    return (valido, "" if valido else "CEP deve conter 8 dígitos")

@registrar_validador("idade")
def validar_idade(valor) -> tuple[bool, str]:
    try:
        valido = 0 <= int(valor) <= 120
    except ValueError:
        valido = False
    return (valido, "" if valido else "Idade deve ser um número entre 0 e 120")

@registrar_validador("nome")
def validar_nome(valor) -> tuple[bool, str]:
    valido = len(str(valor).strip()) > 1
    return (valido, "" if valido else "Nome muito curto")
...
# Saída
print("Atividade 3:")
print(validar_campo_depois("email", "teste@empresa.com"))
print(validar_campo_depois("idade", "25"))
...
# Cálculo de Desconto
desconto_registry.register("cupom:blackfriday", lambda c: (c.valor_total * 0.30, "Cupom Black Friday 30%"))
desconto_registry.register("cliente:vip:alto",  lambda c: (c.valor_total * 0.20, "Cliente VIP Especial 20%"))
desconto_registry.register("cliente:fiel:regular", lambda c: (c.valor_total * 0.10, "Cliente Fiel 10%"))
desconto_registry.register("combo:quantidade", lambda c: (c.valor_total * 0.15, "Desconto Combo Leve Mais 15%"))
desconto_registry.register("fds:padrao", lambda c: (15.0, "Desconto Fim de Semana R$15 fixo"))
desconto_registry.register("novo_cliente", lambda c: (10.0, "Boas-vindas R$10 fixo"))
desconto_registry.register("sem_desconto", lambda c: (0.0, "Sem descontos aplicáveis"))
...
def decidir_regra(carrinho: Carrinho) -> str:
    ...
    if carrinho.quantidade_itens >= 5:
        return "combo:quantidade"
    if carrinho.tipo_cliente == "fiel":
        return "cliente:fiel:regular"
...
def calcular_desconto_depois(carrinho: Carrinho) -> tuple[float, str]:
    chave_regra = decidir_regra(carrinho)
    return desconto_registry.get(chave_regra)(carrinho)

# Saída
print("Atividade 4: ")
carrinho_vip = Carrinho(valor_total=600.0, cupom=None, tipo_cliente="vip", dia_semana=1, quantidade_itens=2)
print(calcular_desconto_depois(carrinho_vip)) # (120.0, 'Cliente VIP Especial 20%')
