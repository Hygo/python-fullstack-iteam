...
Aluno(a): Kelvin Ferreira / Ryan Kayky
=============================================================
...
print(calcular_antes("potencia", 2,  8))         # → 256
...
OPERACOES_REGISTRY: dict = {
    ...
    "raiz":          lambda a, b: math.sqrt(a),   # desafio extra: √a
}
...
# Classe Registry completa — reutilizada nas atividades 3, 4 e 5
class Registry:
    ...
    def register(self, chave: str, valor) -> None:
        if chave in self._store:
            raise KeyError(f"[{self._nome}] Chave '{chave}' já registrada.")
...
def _relatorio_json_pretty(dados: dict) -> None:   # desafio extra
...
# Desafio extra — adicionado sem tocar em validar_campo_depois()
...
# Demonstração do desafio extra — validador de URL
print("\n  [DESAFIO] Validador de URL:")
urls_teste = [
    ("url", "https://iteam.com.br"),
    ("url", "ftp://invalido.com"),
]
for tipo, valor in urls_teste:
    ok, msg = validar_campo_depois(tipo, valor)
    status  = "✅" if ok else "❌"
    print(f"  {status} {tipo:10} | {str(valor):25} | {msg or 'OK'}")
...
# Handlers — cada um recebe Carrinho e retorna (percentual, motivo)
def _desc_cupom_blackfriday(c: Carrinho)       -> tuple[float, str]: return (0.30, "Cupom Black Friday: 30%")
def _desc_cupom_bemvindo_valido(c: Carrinho)   -> tuple[float, str]: return (0.10, "Cupom de boas-vindas: 10%")
def _desc_cupom_bemvindo_invalido(c: Carrinho) -> tuple[float, str]: return (0.0,  "Cupom BEMVINDO10 apenas para novos clientes")
def _desc_vip_alto_valor(c: Carrinho)          -> tuple[float, str]: return (0.20, "VIP + compra >= R$500: 20%")
def _desc_vip_padrao(c: Carrinho)              -> tuple[float, str]: return (0.10, "VIP: 10% base")
def _desc_fiel_muitos_itens(c: Carrinho)       -> tuple[float, str]: return (0.15, "Fiel + 5 itens: 15%")
def _desc_fiel_padrao(c: Carrinho)             -> tuple[float, str]: return (0.08, "Fiel: 8%")
def _desc_fds_alto_valor(c: Carrinho)          -> tuple[float, str]: return (0.12, "FDS + >= R$200: 12%")
def _desc_fds_padrao(c: Carrinho)              -> tuple[float, str]: return (0.05, "Fim de semana: 5%")
def _desc_compra_acima_mil(c: Carrinho)        -> tuple[float, str]: return (0.10, "Compra acima de R$1.000: 10%")
def _desc_padrao(c: Carrinho)                  -> tuple[float, str]: return (0.0,  "Sem desconto aplicável")
def _desc_aniversario_loja(c: Carrinho)        -> tuple[float, str]: return (0.25, "Aniversário da loja: 25%")  # desafio extra

desconto_registry.register("cupom:blackfriday",        _desc_cupom_blackfriday)
desconto_registry.register("cupom:bemvindo:valido",     _desc_cupom_bemvindo_valido)
desconto_registry.register("cupom:bemvindo:invalido",   _desc_cupom_bemvindo_invalido)
desconto_registry.register("cliente:vip:alto_valor",    _desc_vip_alto_valor)
desconto_registry.register("cliente:vip:padrao",        _desc_vip_padrao)
desconto_registry.register("cliente:fiel:muitos_itens", _desc_fiel_muitos_itens)
desconto_registry.register("cliente:fiel:padrao",       _desc_fiel_padrao)
desconto_registry.register("fds:alto_valor",            _desc_fds_alto_valor)
desconto_registry.register("fds:padrao",                _desc_fds_padrao)
desconto_registry.register("compra:acima_mil",          _desc_compra_acima_mil)
desconto_registry.register("aniversario_loja",          _desc_aniversario_loja)
desconto_registry.register("padrao",                    _desc_padrao)


def decidir_regra(carrinho: Carrinho) -> str:
    """Decision Engine — contém toda a lógica condicional e devolve apenas uma chave."""
    if carrinho.cupom == "BLACKFRIDAY":
    ...
    if carrinho.dia_semana == 3 and carrinho.valor_total >= 300:  # desafio extra
...
# Demonstração do desafio extra — quarta + >= R$300
print("\n  [DESAFIO] Aniversário da loja (quarta-feira, R$300+):")
carrinho_niver = Carrinho(350.0, None, "normal", 3, 2)
pct, motivo = calcular_desconto_depois(carrinho_niver)
print(f"  R$ {carrinho_niver.valor_total:7.2f} | {pct*100:4.0f}% | {motivo}")
...
# Registry reutilizado da Atividade 2 (classe Registry já definida acima)
evento_registry = Registry(nome="EventoRegistry")
...
def handle_pagamento_aprovado_v2(evento: Evento) -> dict:
    """V2: acrescenta notificação ao webhook externo."""
    acoes = ["pedido_liberado", "nota_fiscal emitida"]
    if evento.prioridade >= 3:
        acoes.append("notificacao_prioritaria enviada")
    acoes.append("webhook_externo notificado")   # novo comportamento da V2
    return {"evento": evento.tipo, "status": "ok", "acoes": acoes}

evento_registry.update("pagamento.aprovado", handle_pagamento_aprovado_v2)
print("\n[FEATURE FLAG] handler de pagamento.aprovado atualizado em runtime!")

ev_v2 = Evento("pagamento.aprovado", {"valor": 500.0}, 3, "api")
res   = processar_evento_depois(ev_v2)
print(f"  [{res['status'].upper():8}] {ev_v2.tipo:25} → {res['acoes']}")
