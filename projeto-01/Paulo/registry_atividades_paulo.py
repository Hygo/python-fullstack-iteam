...
class Registry:
    ...
    def register(self, chave: str, valor):
...
gerar_relatorio_depois("json_pretty", dados_exemplo)
...
def validar_url(valor) -> tuple[bool, str]:
    valido = (str(valor).startswith("http://") or str(valor).startswith("https://")) and "." in str(valor)
...
ok, msg = validar_campo_depois("url", "https://iteam.com")
status  = "✅" if ok else "❌"
print(f"  {status} {'url':10} | {'https://iteam.com':25} | {msg or 'OK'}")
...
desconto_registry.register("promocao:aniversario_loja", lambda c: (0.25, "Aniversário da Loja: 25%"))
...
def decidir_regra(carrinho: Carrinho) -> str:
    ...
    if carrinho.dia_semana == 3 and carrinho.valor_total >= 300:
        return "promocao:aniversario_loja"
...
c_niver = Carrinho(350.0, None, "normal", 3, 2)
pct, motivo = calcular_desconto_depois(c_niver)
print(f"  R$ {c_niver.valor_total:7.2f} | {pct*100:4.0f}% | {motivo} (TESTE NOVA REGRA)")
