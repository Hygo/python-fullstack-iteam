...
def salvar_nota(turma, aluno, nota1, nota2, nota3, media, media_arredondada, arquivo, registro, caminho_arquivo, dados):
    """
    ...
    caminho_arquivo = os.path.join("notas", f"{turma.replace(' ', '_')}.json")
...
def ler_notas(turma, caminho_arquivo, dados, registro, nota1, nota2, nota3, media):
    """
    ...
    if not os.path.exists(caminho_arquivo): 
        print(f"Ainda não há registros da turma '{turma}'.")
        return
    with open(caminho_arquivo, "r") as arquivo:
        dados = json.load(arquivo)
