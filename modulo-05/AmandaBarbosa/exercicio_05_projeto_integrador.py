...
class Livro(ItemBiblioteca):
    ...
    def info_completa(self):
        ...
        info = (
            ...
            f"Autor:          {self.autor}\n"
            f"ISBN:           {self.isbn}\n"
            f"Ano Publicação: {self.ano_publicacao}\n"
            f"Nº Páginas:     {self.num_paginas}\n"
            f"Status:         {status}\n"
        )
        print(info)
        return info
...
class Pessoa:
    ...
    def registrar_emprestimo(self, item):
        ...
        if not self.pode_emprestar():
            print(f"Falha: O membro {self.nome} atingiu o limite máximo de {self.limite_emprestimos} empréstimos.")
            return False
        if not item.disponivel:
            print(f"Falha: O item '{item.titulo}' já está emprestado.")
            return False

        item.emprestar()
        self._emprestimos_ativos.append(item)
        print(f"Sucesso: Item '{item.titulo}' emprestado para {self.nome}.")
        return True
    ...
    def registrar_devolucao(self, item):
        ...
        if item in self._emprestimos_ativos:
            ...
            print(f"Sucesso: Item '{item.titulo}' devolvido por {self.nome}.")
            return True
        print(f"Falha: O item '{item.titulo}' não consta nos empréstimos ativos de {self.nome}.")
        return False
...
class Biblioteca:
    ...
    def adicionar_item(self, item):
        ...
        if item in self.__acervo:
            print(f"Aviso: O item com código '{item.codigo}' já existe no acervo.")
        else:
            self.__acervo.append(item)
            print(f"Item '[{item.codigo}] {item.titulo}' adicionado com sucesso ao acervo.")
    ...
    def cadastrar_membro(self, pessoa):
        ...
        print(f"Membro '{pessoa.nome}' cadastrado com sucesso.")
    ...
    def realizar_emprestimo(self, membro, codigo_item):
        ...
        if item is None:
            print(f"Falha: Item com o código '{codigo_item}' não foi encontrado no acervo.")
        else:
...
if __name__ == "__main__":
    ...
    print("\n--- Informações Detalhadas (Polimorfismo) ---")
    livro1.info_completa()
    revista.info_completa()

    # TODO (DESAFIO 1): Tente emprestar mais itens do que o limite do Aluno (3)
    print("\n--- Execução do Desafio 1: Estourando Limite de Empréstimos do Aluno ---")
    livro3 = Livro("L003", "Clean Code", 2008, "Robert C. Martin", "978-0132350884", 464)
    livro4 = Livro("L004", "Design Patterns", 1994, "Gang of Four", "978-0201633610", 395)
    biblioteca.adicionar_item(livro3)
    biblioteca.adicionar_item(livro4)
    
    biblioteca.realizar_emprestimo(aluno, "L001") # Pega de volta o L001 (1)
    biblioteca.realizar_emprestimo(aluno, "L002") # Pega L002 (2)
    biblioteca.realizar_emprestimo(aluno, "L003") # Pega L003 (3) -> Limite atingido!
    biblioteca.realizar_emprestimo(aluno, "L004") # Tentativa de pegar o 4º item (Bloqueado)

    # TODO (DESAFIO 2): Tente adicionar o mesmo livro duas vezes ao acervo.
    print("\n--- Execução do Desafio 2: Duplicando Item no Acervo ---")
    livro_duplicado = Livro("L001", "Python para Principiantes - Cópia", 2012, "Eugenia Bahit", "978-0000000001", 220)
    biblioteca.adicionar_item(livro_duplicado)
